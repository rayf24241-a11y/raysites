...
Every page has an Open Graph card in `og/` so links posted to Reddit, Discord, X
or Slack render a real preview instead of a bare URL. They are generated by
`tools/make-og.py` (Pillow + system fonts) — rerun it if you change the copy.

**The `og:image` URLs are absolute and hardcoded to `https://raysites.vercel.app`.**
Scrapers do not run JavaScript and do not reliably resolve relative paths, so if
your deployment lives on a different domain, find-and-replace that base URL
across the HTML files or the previews will 404.

Favicons are inline SVG data URIs — no file, no extra request.
